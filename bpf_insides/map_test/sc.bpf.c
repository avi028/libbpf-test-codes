...
static inline u32 uint8_t2u32(uint8_t s[] , uint16_t size){
    u32 value=0;
    for(int i=0 , j=0; j<size && i < MAX_U32_DIGITS ; i++,j++){
        value = value*10 + (u32)(s[j]-(uint8_t)('0'));
    }
    return value;
}
...
int handle_egress(struct __sk_buff *skb)
{
    ...
    ptr= (pkt_data*)((void*)data + payload_offset) ;

// MAP_UPDATE:
    #if MAP_TYPE == HASHMAP    
    map_key_t * map_key = &(ptr->map_key);
