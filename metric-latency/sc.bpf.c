...
int handle_egress(struct __sk_buff *skb)
{
    ...
    if(((void *) data + payload_offset + sizeof(*tsv)) > data_end)
        goto EXIT;
    tsv = (struct ts *) ((void *)data + payload_offset);

    payload_offset += TIMESTAMP; // 8bytes of timestamp
